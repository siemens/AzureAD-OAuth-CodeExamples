...
namespace MicrosoftGraphAspNetCoreConnectSample.Services
{
    public static class GraphService
    {
        ...
        public static async Task<string> GetUserJson(GraphServiceClient graphClient, string upn, HttpContext httpContext)
        {
            if (upn == null) return JsonConvert.SerializeObject(new { Message = "User principal name cannot be null." }, Formatting.Indented);
            ...
            catch (ServiceException e)
            {
                switch (e.Error.Code)
                {
                    ...
                    case "itemNotFound":
                        return JsonConvert.SerializeObject(new { Message = $"User '{upn}' was not found." }, Formatting.Indented);
                    case "ErrorInvalidUser":
                        return JsonConvert.SerializeObject(new { Message = $"The requested user '{upn}' is invalid." }, Formatting.Indented);
        ...
        public static async Task<string> GetPictureBase64(GraphServiceClient graphClient, string upn, HttpContext httpContext)
        {
            try
            {
                // Load user's profile picture.
                var pictureStream = await GetPictureStream(graphClient, upn, httpContext);
            ...
            catch (Exception e)
            {
                return e.Message switch
                {
                    ...
                    "UpnIsNull" => JsonConvert.SerializeObject(new { Message = "Email address cannot be null." }, Formatting.Indented),
                    _ => null,
                };
            }
        }

        public static async Task<Stream> GetPictureStream(GraphServiceClient graphClient, string upn, HttpContext httpContext)
        {
            if (upn == null) throw new Exception("UpnIsNull");
            ...
            try
            {
                ...
                {
                    // Load user's profile picture.
                    pictureStream = await graphClient.Users[upn].Photo.Content.Request().GetAsync();
                }
                catch (ServiceException e)
                {
